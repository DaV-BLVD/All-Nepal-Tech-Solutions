        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                    <i class="fas fa-star mr-2 text-indigo-600"></i> Why Choose Us
                </h1>
                <p class="text-sm text-gray-500 mt-1">Manage your Unique Selling Propositions and competitive advantages.</p>
            </div>

            {{-- Add New Button --}}
            <a href="{{ route('why_choose_us.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <thead class="bg-primary text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-widest">USP Title</th>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="h-12 w-12 flex items-center justify-center rounded-xl bg-indigo-50 border border-indigo-100 shadow-sm">
                                        <i class="{{ $usp->icon ?? 'fas fa-check' }} text-2xl text-indigo-600"></i>
                                    </div>
                                </td>

                                {{-- Title --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-gray-900 uppercase tracking-tight">{{ $usp->title }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center space-x-3">
                                        <a href="{{ route('why_choose_us.edit', $usp) }}"
                                            class="p-2 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-lg transition-all"
                                            title="Edit USP">
                                            <i class="fas fa-edit"></i>
                                        </a>

                                        <form action="{{ route('why_choose_us.destroy', $usp) }}" method="POST"
                                            onsubmit="return confirm('Are you sure you want to delete this USP?');"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 font-medium">No selling points defined yet.</p>
                                        <a href="{{ route('why_choose_us.create') }}"
                                            class="mt-2 text-indigo-600 hover:underline text-sm">Add your first USP</a>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
